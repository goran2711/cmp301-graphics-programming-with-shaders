...
D3D::D3D(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, deviceFlags, &featureLevel, 1, D3D11_SDK_VERSION, &swapChainDesc, &swapChain, &device, NULL, &deviceContext);
	...
	device->CreateShaderResourceView(backBufferPtr, &srvDesc, &renderTargetResourceView);
	...
	device->CreateUnorderedAccessView(backBufferPtr, &uavDesc, &renderTargetUnorderedAccessView);
	...
	device->CreateShaderResourceView(depthStencilBuffer, &depthSRVDesc, &depthShaderResourceView);
...
XMMATRIX D3D::getProjectionMatrix() const
{
	return projectionMatrix;
}


XMMATRIX D3D::getWorldMatrix() const
{
	return worldMatrix;
}


XMMATRIX D3D::getOrthoMatrix() const
{
	return orthoMatrix;
}

ID3D11ShaderResourceView* D3D::getShaderResourceView() const
{
	return renderTargetResourceView;
}

ID3D11ShaderResourceView * D3D::getDepthShaderResourceView() const
{
	return depthShaderResourceView;
}

ID3D11UnorderedAccessView* D3D::getUnorderedAccessView() const
{
	return renderTargetUnorderedAccessView;
}
...
bool D3D::getZBufferState() const
{
	return zbufferState;
}
...
bool D3D::getAlphaBlendingState() const
{
...
bool D3D::getWireframeState() const
{
