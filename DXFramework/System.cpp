...
void System::initialiseWindows(int& screenWidth, int& screenHeight)
{
	...
	RECT viewportRect;
	viewportRect.top = viewportRect.left = 0;
	viewportRect.right = screenWidth;
	viewportRect.bottom = screenHeight;

	DWORD wsFlags = WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_OVERLAPPEDWINDOW;

	AdjustWindowRect(&viewportRect, wsFlags, false);

	// Create the window with the screen settings and get the handle to it.
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, applicationName, applicationName,
		wsFlags,
		posX, posY, viewportRect.right - viewportRect.left, viewportRect.bottom - viewportRect.top, NULL, NULL, hinstance, NULL);
